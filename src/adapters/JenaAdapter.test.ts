...
import { RdfFormat, RdfTermType, SelectQueryResult } from '../core/types';
import { JenaAdapter } from './JenaAdapter';

describe('JenaAdapter (Integration Tests)', () => {
  ...
  beforeAll(async () => {
    // Check if Jena Fuseki server is available
    try {
      const response = await axios.get('http://localhost:3030/$/ping', { timeout: 5000 });
      if (response.status !== 200) {
        throw new Error('Server not responding');
      }
    } catch (error) {
      console.warn('Jena Fuseki server not available, skipping integration tests');
    ...
    // Create real adapter
    adapter = new JenaAdapter(endpoint, dataset);
  ...
  describe('query', () => {
    ...
    it('should execute SELECT query successfully', async () => {
      ...
      expect((result as SelectQueryResult).results.bindings).toHaveLength(1);
    });
